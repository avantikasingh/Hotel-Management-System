	...
	public boolean register(Customer customer) {
		...
		for(Customer customer2:list) {
			if(customer2.getUsername().equals(customer.getUsername()))
				throw new HotelException("Username should be unique");
		}
...
//			throw new HotelException("CheckIn date should be greater than current date");
	...
	@Override
	public Map<Hotel,List<Room>> availableRooms(LocalDate checkin, LocalDate checkout, Long cityId) {
		...
		if(cityList.size()==0)
			throw new ResourceNotFoundException("No cities in database");
