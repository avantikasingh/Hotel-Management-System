...
public class HotelManagementService implements IHotelManagementService {
    ...
    public boolean addRoom(int cityId,int hotelId,int roomId,String roomType,double roomRent,String roomNumber) {
    	...
    	Room newRoom = new Room(roomId,roomType,roomRent,roomNumber,null);		//create Hotel object
    ...
    public boolean showHotel(int cityId) {
        return hotelManagementDao.showHotel(cityId);
    }

    @Override
    public boolean showRoom(int cityId, int roomId) {
        return hotelManagementDao.showRoom(cityId, roomId);
    }

    @Override
    public Map<Integer, Hotel> viewHotel(Date checkIn, Date checkOut, int cityId, boolean sortByPrice) {
        return hotelManagementDao.viewHotel(checkIn,checkOut,cityId,sortByPrice);
    }

    @Override
    public boolean addBooking(int cityId, int hotelId, int roomId, Booking booking) {
        return hotelManagementDao.addBooking(cityId, hotelId, roomId, booking);
    }
